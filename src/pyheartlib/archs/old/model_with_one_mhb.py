...
class MultiHeadAttentionClassifier(tf.keras.Model):
    ...
    def call(self, x, training):
        ...
        #norm_out1 = attn_out1 + x
        ##norm_out1 = self.bn2(attn_out1 + x)    #(batch_size, seq_len, d_model)
        ...
        #--------------------------------------------------------------------------
        norm_out1 = self.flatten(x)   #(batch_size, seq_len*d_model)
        ...
        norm_out1 = self.dense1(norm_out1)
        norm_out1 = self.dropout2(norm_out1, training=training) 
        norm_out1 = self.dense2(norm_out1)
        norm_out1 = self.dropout3(norm_out1, training=training) 
        norm_out1 = self.dense3(norm_out1)
