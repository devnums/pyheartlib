...
class EmbeddingLayer(tf.keras.layers.Layer):
    ...
    def build(self,input_shape):
      ...
      self.emded_dim = d_model

      self.embedding = tf.keras.layers.Embedding(sig_res, d_model)
      #self.position_emb = tf.keras.layers.Embedding(input_dim=self.seq_len, output_dim=self.emded_dim)
      self.pos_encoding = positional_encoding(maximum_position_encoding=1000, self.emded_dim) 

    def call(self, x):
      # x is the input to the network (...,seq_len,emded_dim) 
      positions_emb = self.pos_encoding[:, :self.seq_len, :] 
